...
function saveNote(cell, text) {
  ...
  // Emptying a note is a deletion, and a deletion should look immediate. The
  // POST is debounced 600ms; waiting for it means you watch the pip sit there
  // on a cell you just cleared. Drop it now and let the write catch up. The
  // catch below repaints from `notes` if the server refuses, so a failed save
  // puts the pip back rather than silently losing it.
  if (!text.trim() && notes[cell] !== undefined) {
    delete notes[cell];
  ...
  saveTimer = setTimeout(async () => {
    try {
      ...
      setNoteStatus('not saved — is the server running?');
      // We may have already taken the pip away. The note still exists as far
      // as the vault is concerned, so put the map back the way the vault says.
      loadNotes();
    }
  }, 600);
}
