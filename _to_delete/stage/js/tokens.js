...
// --- shared state ---
let tokens = [];                 // tokens ACTIVE ON THE CURRENT MAP; mirrored on projector/remote
...
let roster = [];
let tokenMapSrc = null;          // map src whose placements are currently hydrated
let tokensReady = false;         // guards saves until loadTokens() has run
...
let playerRenderXform = null;    // set by renderPlayerFogDoubleBuffered

// remote-page state
let isRemoteView = false;
...
function mapDims() {
  return {
    mw: mapWidth || (remoteFrame && remoteFrame.width) || 1,
    mh: mapHeight || (remoteFrame && remoteFrame.height) || 1
  };
}
...
function cellAtMapPx(mx, my, mw, mh) {
...
function cellCenterMapPx(cell, mw, mh) {
  const d = mapDims(); mw = mw || d.mw; mh = mh || d.mh;
  if (isHexKey()) {
...
function maybeSeedDeltaGreenAgents() {
  const dg = campaigns.find(c => c.slug === 'delta-green');
  if (!dg) return;
  ...
  migrateLegacyTokens('delta-green',
    activeCampaign === 'delta-green' ? lastMapSrc : null);
...
function mapKeyStorageKey(src) { return gameKey('key:' + relMapSrc(src || '__no-map__')); }

function saveMapKey() {
  if (isPlayerView || isRemoteView || !lastMapSrc) return;
  try {
    localStorage.setItem(mapKeyStorageKey(lastMapSrc), JSON.stringify({
...
function applySavedMapKey(src) {
  if (isPlayerView || isRemoteView) return;
  let k = null;
...
function hydrateTokensForMap(src) {
  ...
  tokenMapSrc = src ? relMapSrc(src) : null;
  let placements = {};
  try {
    const raw = localStorage.getItem(mapTokensKey(tokenMapSrc));
...
function setTokenMap(src) {
  if (isPlayerView || isRemoteView || !tokensReady) return;
  src = src ? relMapSrc(src) : null;
  if (src === tokenMapSrc) return;
...
function addRosterTokenToMap(id) {
  const d = roster.find(t => t.id === id);
  if (!d || tokens.includes(d)) return;
  const c = snapNorm(0.5, 0.5);
  d.tx = c.tx; d.ty = c.ty; d.onMap = false; d.pending = null;
  tokens.push(d);
  onTokensChanged();
}
function addRosterTokenFromSelect() {
...
function duplicateToken(id) {
  ...
  if (!src) return;
  const group = roster.filter(t => t.base === src.base);
...
function renderRosterAddOptions() {
  ...
  const avail = roster.filter(t => t.side === 'npc' && !activeIds.has(t.id));
...
function refreshTokenImageOptions() {
...
function setKeyCell(v) {
  keyCellPx = Math.max(2, parseFloat(v) || 0);
  if (keyLinkXY) keyCellYPx = 0;
  resnapAllTokens();
  keyChanged();
}
function nudgeKeyCell(d) { setKeyCell(kCellW(mapWidth) + d); }
...
function setKeyAcross(v) {
  const n = parseFloat(v);
  if (!n || n <= 0 || !mapWidth) return;
  setKeyCell(mapWidth / n);
}
function setKeyCellY(v) { keyCellYPx = Math.max(2, parseFloat(v) || 0); resnapAllTokens(); keyChanged(); }
function nudgeKeyCellY(d) { setKeyCellY(kStepY(mapWidth) + d); }
function toggleKeyLink() {
  keyLinkXY = document.getElementById('key-link').checked;
  if (keyLinkXY) keyCellYPx = 0; else keyCellYPx = kStepY(mapWidth);
  resnapAllTokens();
  keyChanged();
}
function setKeyOx(v) { keyOx = parseFloat(v) || 0; resnapAllTokens(); keyChanged(); }
function setKeyOy(v) { keyOy = parseFloat(v) || 0; resnapAllTokens(); keyChanged(); }
function nudgeKeyOrigin(dx, dy, step) {
  step = step || 1;
...
function syncKeyPanel() {
  ...
  const across = mapWidth ? (mapWidth / cw) : 0;
...
function renderGMTokens() {
  ...
  if (currentMode !== 'fog' || fogContext !== 'map') return;
  const sizePct = (kTokenDiam(mapWidth) / (mapWidth || 1)) * 100 * TOKEN_FRAC;
  // Draw only tokens the GM has made visible on this map (onMap is per-map).
  tokens.filter(t => t.onMap).forEach(t => {
...
function postMarker(stroke) {
  ...
  if (mapChannel) mapChannel.postMessage({ type: 'markers', markers: Object.values(activeMarkers) });
}
...
function pushPlayerStateToServer() {
  ...
  _serverFrameTimer = setTimeout(() => {
    ...
    const payload = {
      ...
      imageSrc: relMapSrc(fogImage.src),
      width: mapWidth, height: mapHeight,
      fogRLE: rlEncode(fogMask),
      crop: viewportCrop,
      grid: gridWirePayload()
    };
...
function saveTokens() {
  ...
  try {
    ...
    const placements = {};
    tokens.forEach(t => { placements[t.id] = { tx: t.tx, ty: t.ty, onMap: !!t.onMap }; });
    localStorage.setItem(mapTokensKey(tokenMapSrc), JSON.stringify({ placements }));
  } catch (e) { console.warn('save tokens failed', e); }
}
function loadTokens() {
  tokensReady = false;
  roster = []; tokens = [];
  migrateLegacyTokens(activeCampaign, lastMapSrc);
  try {
    const raw = localStorage.getItem(campaignKey('roster'));
    if (raw) {
      const s = JSON.parse(raw);
      roster = Array.isArray(s.roster) ? s.roster : [];
      roster.forEach(t => { t.pending = null; if (t.owner === undefined) t.owner = ''; });
  ...
  hydrateTokensForMap(lastMapSrc);
  tokensReady = true;
}
...
// ===================================================================
function projectorHandleMessage(d) {
  if (d.type === 'tokens') {
    pTokens = d.tokens || [];
...
// ===================================================================
// REMOTE PLAYER PAGE  (?mode=remote)
// ===================================================================
...
function attemptRemoteRestore() {
  ...
  const t = tokens.find(x => x.id === savedClaim);
...
function remoteChangeCharacter() {
...
function renderRemoteTokens() {
  ...
  const mine = tokens.find(t => t.id === remoteClaimId);
