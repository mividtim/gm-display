...
function renderPlayerImage(img, crop) {
  ...
  if (pRotation) ctx.rotate(pRotation * Math.PI / 180);
...
function renderPlayerFogDoubleBuffered(data) {
  ...
  if (!offscreenCanvas || offscreenCanvas.width !== preW || offscreenCanvas.height !== preH) {
    offscreenCanvas = document.createElement('canvas');
    offscreenCanvas.width = preW;
    offscreenCanvas.height = preH;
    offscreenCtx = offscreenCanvas.getContext('2d');
  }

  const doRender = (img) => {
    const ctx = offscreenCtx;
    const [bgR, bgG, bgB] = parseHexColor(pBg);
    ctx.fillStyle = pBg;
    ...
    visCtx.fillStyle = pBg;
    visCtx.fillRect(0, 0, wW, wH);
    visCtx.save();
    visCtx.translate(wW / 2, wH / 2);
    if (pRotation) visCtx.rotate(pRotation * Math.PI / 180);
    visCtx.drawImage(offscreenCanvas, -preW / 2, -preH / 2);
    visCtx.restore();
    ...
    playerRenderXform = { wW, wH, scale, srcX, srcY, preW, preH, rot: pRotation, mapW: data.width, mapH: data.height };
    if (playerDisplay === 'map') { drawProjectorOverlay(); drawAllMarkers(); }
  };
