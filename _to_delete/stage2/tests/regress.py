...
async def capture(pg, results):
    ...
    put('ui.handlers', await pg.evaluate("""(() => {
        const names = new Set();
        document.querySelectorAll('*').forEach(el => {
          for (const a of el.attributes || [])
            if (a.name.startsWith('on'))
              (a.value.match(/([A-Za-z_$][\\w$]*)\\s*\\(/g) || [])
                .forEach(m => { const n = m.slice(0, -1).trim();
                  if (!/^(getElementById|rgba|click|parseInt|parseFloat|Number|String)$/.test(n)) names.add(n); });
        });
        return { referenced: names.size,
                 missing: [...names].filter(n => typeof window[n] !== 'function').sort() };
      })()"""))
...
async def run(out_path):
    ...
    async with async_playwright() as p:
        ...
        await pg.goto(BASE + '/gm_display.html')
        ...
        for label, url in [('projector', '/gm_display.html?mode=player&display=map'),
                           ('sidecar',   '/gm_display.html?mode=player&display=show'),
                           ('remote',    '/gm_display.html?mode=remote')]:
            ...
            results[f'role.{label}'] = await p2.evaluate("[isPlayerView, isRemoteView, playerDisplay || null]")
            await p2.close()
        await b.close()
