...
`publish-list.js` regenerates the `const PUBLIC = {...}` line in `js/gm.js` from
what the modules actually export. That list is only the `window.GMD` debug/test
handle, but keeping it by hand means a new export stays invisible to
`tests/regress.py` until someone remembers to add it — which is how the party
notes probe first came back "partyCount is not defined". Entry points and
`bindings.js` are excluded deliberately: publishing `attachRemoteBindings` from
the GM page drags the other pages' wiring into it and kills the page on load.

    node tools/publish-list.js && node tools/fix-imports.js
